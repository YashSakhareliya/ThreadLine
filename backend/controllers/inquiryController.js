...
export const getTailorInquiries = async (req, res) => {
  try {
    ...
    // Fetch inquiries excluding closed ones
    const inquiries = await Inquiry.find({ 
      tailor: tailorId,
      status: { $ne: 'closed' } // Exclude closed inquiries
    })
...
// @route   PATCH /api/v1/tailors/:id/inquiries/:inquiryId/read
// @access  Private (Tailor owner)
export const markInquiryAsRead = async (req, res) => {
  try {
    ...
    // await emailService.sendReadConfirmation({
...
// @desc    Close inquiry
// @route   PATCH /api/v1/tailors/:id/inquiries/:inquiryId/close
// @access  Private (Tailor owner)
